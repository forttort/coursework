# SITE MVP

## Что добавлено

Минимальный сайт для отображения данных без сложной верстки.

### Backend
- `backend/main.py`
- `backend/requirements.txt`

### Frontend
- `frontend/index.html`
- `frontend/product.html`
- `frontend/app.js`
- `frontend/product.js`
- `frontend/styles.css`

### Data source
- `rinkan_products_v4.json`

Сейчас JSON-файл содержит `10` товаров.

## Как запустить

### 1. Установить зависимости
```bash
pip install -r backend/requirements.txt
```

### 2. Запустить сервер
```bash
uvicorn backend.main:app --reload
```

### 3. Открыть в браузере
```text
http://127.0.0.1:8000
```

## Доступные маршруты

### HTML
- `/` — каталог
- `/product.html?id=<source_product_id>` — страница товара

### API
- `/api/health`
- `/api/products`
- `/api/products/{source_product_id}`

Поддерживаемые query-параметры каталога:
- `q`
- `general_category`
- `brand`

## Что сейчас использует backend
Сейчас backend умеет:
- читать PostgreSQL, если настроено подключение
- fallback'аться на `rinkan_products_v4.json`, если БД нет

Это позволяет не ломать локальный запуск даже без поднятой БД.

## Что уже умеет frontend
- показать каталог
- искать по строке
- фильтровать по общей категории
- фильтровать по бренду
- фильтровать по статусу
- листать страницы каталога
- открыть карточку товара
- показать галерею изображений и описание

## Что уже умеет backend вокруг каталога
- читать товары из PostgreSQL
- fallback'аться на JSON
- загружать товары через loader
- обновлять статусы товаров через отдельный refresh script

## Что заменить позже
После полного перевода проекта на БД нужно:
1. использовать PostgreSQL как основной источник данных без fallback-режима для production
2. добавить пагинацию
3. добавить фильтры по бренду, состоянию, размеру
4. добавить price_rub и delivery_estimate
