...
v3:
- исправлено извлечение brand_name
- исправлено извлечение condition_text
- возвращено извлечение description через точечные label-блоки
"""
...
def extract_brand_name(lines: list[str], brand_top: Optional[str]) -> Optional[str]:
    slash_candidates: list[str] = []

    for line in lines:
        if "/" not in line and "／" not in line:
            continue
        candidate = normalize_brand_name(line)
        if not candidate:
            continue
        if len(candidate) > 60:
            continue
        if candidate.lower() == "askwatch online":
            continue
        slash_candidates.append(candidate)

    if slash_candidates:
        return slash_candidates[-1]

    candidate = normalize_brand_name(brand_top)
    if candidate and len(candidate) <= 60:
        return candidate

    for line in lines:
        short_line = clean_text(line)
        if not short_line:
            continue
        if len(short_line) > 40:
            continue
        if re.fullmatch(r"[A-Za-z][A-Za-z0-9 .&'’+\-]{1,39}", short_line):
            if short_line.lower() != "askwatch online":
                return short_line
...
def parse_detail_page(page_url: str, session: requests.Session) -> RawRinkanProduct:
    ...
    brand_name = extract_brand_name(lines, brand_top)
    condition_text = extract_block_after_label(lines, "商品の状態")
    size_text = normalize_size_text(
        extract_block_after_label(lines, "サイズ") or extract_block_after_label(lines, "実寸")
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="MVP-парсер товаров RINKAN v3: получает ссылки из listing и разбирает detail pages."
    )
    parser.add_argument(
        "--start-url",
        required=True,
    ...
    parser.add_argument(
        "--output",
        default="rinkan_products_v3.json",
